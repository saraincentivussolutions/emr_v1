...
class PDF extends FPDF
{
	function printpt($PDFFormat='')
	{
		...
		if($PDFFormat) $tot = 25; else $tot = 55;
			for($j=0;$j<$tot;$j++)
			{
				for($i=0;$i<35;$i++)
				{
					$this->Write(1,round($this->x));
					$this->SetX(round($this->x)+5);
					$p=$p+5;
				}
			$this->SetY(round($this->y)+5);
			$this->Write(1,round($this->y));
